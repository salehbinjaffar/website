...
const ICONS = {
  facebook: '<svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073c0 6.027 4.388 11.02 10.125 11.91v-8.385H7.078v-3.525h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.525h-2.796v8.385C19.612 23.093 24 18.027 24 12.073z"/></svg>',
...
function socialIconsHtml(social, className = "social-icon-link") {
  if (!social) return "";
  return Object.keys(ICONS)
    .filter((k) => social[k])
    .map((k) => {
      const href = k === "whatsapp" && !String(social[k]).startsWith("http")
        ? `https://wa.me/${String(social[k]).replace(/\D/g, "")}`
        : social[k];
      const platformClass = className === "social-icon-link" ? `social-icon-link ${k}` : className;
      return `<a class="${platformClass}" href="${escapeHtml(href)}" target="_blank" rel="noopener noreferrer" title="${LABELS[k]}">${ICONS[k]}<span class="sr-only">${LABELS[k]}</span></a>`;
    })
    .join("");
}

module.exports = { socialIconsHtml, ICONS };
